...
class Day22Spec extends Specification {
  "Day 22 Specification".br

  val shortSteps = parseSteps(
  ...
  "Naive way for the small example" >> {
    val cubes = naiveReboot(shortSteps)
    cubes.size mustEqual 39
  }

  "Naive way for the large example" >> {
    val cubes = naiveReboot(steps.filter(_.initializationRegion))
    cubes.size mustEqual 590784
  }
}
